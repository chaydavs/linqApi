...
from tiles.renderer import _base_css, _tile_inner_html, render_tile_html, TILE_WIDTH, TILE_HEIGHT # type: ignore
...
class TestBaseCss:
    ...
    def test_accent_color_in_css(self) -> None:
        css: str = _base_css("#FF5733")
        assert "#FF5733" in css

    def test_tile_width_in_css(self) -> None:
        css: str = _base_css("#A78BFA")
        assert f"{TILE_WIDTH}px" in css

    def test_tile_height_in_css(self) -> None:
        css: str = _base_css("#A78BFA")
        assert f"{TILE_HEIGHT}px" in css

    def test_tile_class_defined(self) -> None:
        css: str = _base_css("#A78BFA")
        assert ".tile" in css

    def test_tag_class_defined(self) -> None:
        css: str = _base_css("#A78BFA")
        assert ".tag" in css

    def test_headline_class_defined(self) -> None:
        css: str = _base_css("#A78BFA")
        assert ".headline" in css

    def test_stat_class_defined(self) -> None:
        css: str = _base_css("#A78BFA")
        assert ".stat" in css
    ...
    def test_cta_button_class_defined(self) -> None:
        css: str = _base_css("#A78BFA")
        assert ".cta-button" in css

    def test_dm_sans_font_imported(self) -> None:
        css: str = _base_css("#A78BFA")
    ...
    def test_different_accent_produces_different_css(self) -> None:
        css1: str = _base_css("#FF0000")
        css2: str = _base_css("#00FF00")
        assert css1 != css2

    def test_overflow_hidden_on_body(self) -> None:
        css: str = _base_css("#A78BFA")
...
class TestTileInnerHtmlCover:
    def _cover(self, **kwargs: str) -> str:
        base: dict[str, str] = {"type": "cover", "accent": "#A78BFA", "headline": "Test Headline", "body": "Body text"}
        return _tile_inner_html({**base, **kwargs})

    def test_headline_in_output(self) -> None:
        html: str = self._cover()
    ...
    def test_tag_rendered_when_present(self) -> None:
        html: str = self._cover(tag="THE CHALLENGE")
        assert "THE CHALLENGE" in html
        assert 'class="tag"' in html

    def test_tag_absent_when_not_provided(self) -> None:
        html: str = self._cover()
        assert 'class="tag"' not in html

    def test_swipe_indicator_present(self) -> None:
        html: str = self._cover()
        assert "SWIPE" in html

    def test_accent_color_in_output(self) -> None:
        html: str = self._cover(accent="#DEADBE")
        assert "#DEADBE" in html

    def test_headline_class_used(self) -> None:
        html: str = self._cover()
...
class TestTileInnerHtmlStat:
    def _stat(self, **kwargs: str) -> str:
        base: dict[str, str] = {"type": "stat", "accent": "#38BDF8", "stat": "47%",
                "stat_label": "of deals stall", "source": "Gartner 2024"}
        return _tile_inner_html({**base, **kwargs})

    def test_stat_value_in_output(self) -> None:
        assert "47%" in self._stat()

    def test_stat_label_in_output(self) -> None:
        assert "of deals stall" in self._stat()

    def test_source_in_output(self) -> None:
        assert "Gartner 2024" in self._stat()

    def test_stat_class_used(self) -> None:
        assert 'class="stat"' in self._stat()

    def test_missing_stat_does_not_crash(self) -> None:
        html: str = _tile_inner_html({"type": "stat", "accent": "#38BDF8"})
...
class TestTileInnerHtmlList:
    def _list_tile(self, **kwargs: object) -> str:
    ...
    def test_items_rendered(self) -> None:
        html: str = self._list_tile()
        assert "ITEM 1" in html
        assert "First warning" in html

    def test_icon_rendered(self) -> None:
        html: str = self._list_tile()
        assert "⚠️" in html

    def test_empty_items_does_not_crash(self) -> None:
        html: str = _tile_inner_html({"type": "list", "accent": "#A78BFA", "items": []})
        assert isinstance(html, str)

    def test_gain_type_uses_same_template(self) -> None:
        """'gain' type shares the list template."""
        tile: dict[str, object] = {"type": "gain", "accent": "#A78BFA", "headline": "What You Gain",
...
class TestTileInnerHtmlComparison:
    def _comp(self, **kwargs: object) -> str:
        base: dict[str, object] = {
            "type": "comparison",
            "accent": "#64748B",
    ...
    def test_old_value_in_output(self) -> None:
        assert "6 weeks" in self._comp()

    def test_new_value_in_output(self) -> None:
        assert "3 days" in self._comp()

    def test_label_in_output(self) -> None:
        assert "Onboarding" in self._comp()

    def test_line_through_on_old_value(self) -> None:
        assert "line-through" in self._comp()

    def test_metrics_type_shares_comparison_template(self) -> None:
...
class TestTileInnerHtmlQuote:
    def _quote(self, **kwargs: str) -> str:
        base: dict[str, str] = {
            "type": "quote",
            "accent": "#4ADE80",
    ...
    def test_body_quote_in_output(self) -> None:
        assert "We cut our close rate" in self._quote()

    def test_headline_speaker_in_output(self) -> None:
        assert "VP Sales, Notion" in self._quote()

    def test_source_in_output(self) -> None:
        assert "Q3 2024" in self._quote()

    def test_italic_style_on_quote(self) -> None:
...
class TestTileInnerHtmlMath:
    def test_stat_in_output(self) -> None:
        tile: dict[str, str] = {"type": "math", "accent": "#38BDF8", "tag": "YOUR NUMBERS",
                "stat": "$240K", "stat_label": "annual cost of status quo", "source": "5 AEs * $48K"}
        html: str = _tile_inner_html(tile)
        assert "$240K" in html

    def test_tag_in_output(self) -> None:
        tile: dict[str, str] = {"type": "math", "accent": "#38BDF8", "tag": "YOUR NUMBERS", "stat": "$240K"}
        html: str = _tile_inner_html(tile)
...
class TestTileInnerHtmlTimeline:
    def _timeline(self) -> dict[str, object]:
        return {
            "type": "timeline",
            "accent": "#38BDF8",
    ...
    def test_all_items_rendered(self) -> None:
        html: str = _tile_inner_html(self._timeline())
        assert "Week 1" in html
        assert "Month 1" in html
        assert "Month 3" in html

    def test_checkmark_icon_present(self) -> None:
        assert "✓" in _tile_inner_html(self._timeline())

    def test_last_item_has_no_connector_line(self) -> None:
...
class TestTileInnerHtmlPersonal:
    def test_emoji_in_output(self) -> None:
        tile: dict[str, str] = {"type": "personal", "accent": "#F59E0B",
                "stat": "🏈", "headline": "Fellow Orange Nation fan", "body": "We talked for an hour."}
        html: str = _tile_inner_html(tile)
        assert "🏈" in html
        assert "Fellow Orange Nation fan" in html

    def test_default_emoji_when_stat_missing(self) -> None:
        tile: dict[str, str] = {"type": "personal", "accent": "#F59E0B", "headline": "Something personal"}
        html: str = _tile_inner_html(tile)
        assert "👋" in html
...
class TestTileInnerHtmlBridge:
    def test_body_in_output(self) -> None:
        tile: dict[str, str] = {"type": "bridge", "accent": "#F59E0B",
...
class TestTileInnerHtmlFallback:
    def test_unknown_type_renders_headline_and_body(self) -> None:
        tile: dict[str, str] = {"type": "unknown_future_type", "headline": "Mystery Tile",
                "body": "Some body text", "accent": "#A78BFA"}
        html: str = _tile_inner_html(tile)
        assert "Mystery Tile" in html
        assert "Some body text" in html

    def test_returns_string_for_empty_tile(self) -> None:
        html: str = _tile_inner_html({})
        assert isinstance(html, str)
...
class TestRenderTileHtml:
    def _minimal_tile(self, deck_type: str = "hook", tile_index: int = 0) -> str:
        tile: dict[str, str] = {"type": "cover", "headline": "Test", "body": "Body", "accent": "#A78BFA"}
    ...
    def test_tile_div_present(self) -> None:
        assert 'class="tile"' in self._minimal_tile()

    def test_style_tag_present(self) -> None:
        assert "<style>" in self._minimal_tile()

    def test_headline_content_in_page(self) -> None:
        tile: dict[str, str] = {"type": "cover", "headline": "Big Bold Claim", "accent": "#A78BFA"}
        html: str = render_tile_html(tile, deck_type="hook", tile_index=0)
        assert "Big Bold Claim" in html

    def test_accent_color_injected(self) -> None:
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": "#ABCDEF"}
        html: str = render_tile_html(tile, deck_type="hook", tile_index=0)
        assert "#ABCDEF" in html

    # Gradient selection per deck type
    @pytest.mark.parametrize("deck_type", ["hook", "roi", "proof", "personal", "competitive"])  # type: ignore[misc]
    def test_gradient_for_each_deck_type(self, deck_type: str) -> None:
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": ACCENT_COLORS[deck_type]}
        html: str = render_tile_html(tile, deck_type=deck_type, tile_index=0)
        expected_gradient: str = DECK_GRADIENTS[deck_type][0]
        assert expected_gradient in html

    def test_tile_index_selects_correct_gradient(self) -> None:
        """tile_index=1 should use the second gradient."""
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": "#A78BFA"}
        html: str = render_tile_html(tile, deck_type="hook", tile_index=1)
        assert DECK_GRADIENTS["hook"][1] in html

    def test_tile_index_clamps_at_last_gradient(self) -> None:
        """tile_index beyond list length should use the last gradient."""
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": "#A78BFA"}
        hook_gradients: list[str] = DECK_GRADIENTS["hook"]
        html: str = render_tile_html(tile, deck_type="hook", tile_index=999)
        assert hook_gradients[-1] in html

    def test_tile_index_zero_uses_first_gradient(self) -> None:
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": "#A78BFA"}
        html: str = render_tile_html(tile, deck_type="hook", tile_index=0)
        assert DECK_GRADIENTS["hook"][0] in html

    def test_unknown_deck_type_falls_back_to_hook_gradient(self) -> None:
        """An unrecognized deck_type should fall back to hook gradients."""
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": "#A78BFA"}
        html: str = render_tile_html(tile, deck_type="nonexistent_type", tile_index=0)
        assert DECK_GRADIENTS["hook"][0] in html

    def test_body_tag_present(self) -> None:
        html: str = self._minimal_tile()
        assert "<body>" in html
        assert "</body>" in html
    ...
    def test_default_deck_type_is_hook(self) -> None:
        """Default deck_type parameter should produce hook gradients."""
        tile: dict[str, str] = {"type": "cover", "headline": "H", "accent": "#A78BFA"}
        html: str = render_tile_html(tile)
        assert DECK_GRADIENTS["hook"][0] in html
